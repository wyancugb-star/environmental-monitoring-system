...
```c
/// @brief One-time LCD init (FSMC timing, NT35510 init sequence) — thin wrapper over ATK's LCD driver from 03-lcd-fsmc-usmart.
...
- **Anti-flicker design decision:** `lcd_disp_update()` does **not** clear-and-redraw the whole screen every second. It tracks the previously-drawn strings internally (static buffers) and only overwrites the character cells that differ. A full-screen clear at 1 Hz on a parallel FSMC LCD is the single most common cause of visible flicker in this class of project — worth calling out explicitly since it's a direct, non-obvious link back to REQ-NFR-003.
- Traces to: REQ-DISP-001, REQ-DISP-002, REQ-NFR-003.

**Progress bar visualization (both light and temperature):**

```c
/**
 * @brief  Draw/update a horizontal bar with partial-redraw (no flicker).
 * @param  x, y, width, height  Bar position and size on screen
 * @param  value_pct            0-100, already normalized by caller
 * @param  color                Fill color (caller decides, see below)
 */
void lcd_disp_draw_bar(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t value_pct, uint16_t color);
```

- A single generic bar-drawing component is reused for both readings rather than writing two separate implementations — the only difference is the input value and color, not the drawing logic.
- **Light:** `light_pct` is already 0-100%, passed directly. Fixed fill color (e.g. yellow/white).
- **Temperature:** normalized from `[-10°C, 50°C]` (the ambient range assumed in `requirements_spec.md` §7.1) to `[0,100]` before calling `lcd_disp_draw_bar()`. Fill color is **dynamic**, chosen by the caller based on the raw (un-normalized) temperature: below 18°C → blue, 18–28°C → green (comfort range), above 28°C → red. This gives the temperature bar an at-a-glance "is this in a normal range" read, not just a magnitude.
- Same partial-redraw principle as the rest of §3.3 applies: only the delta region of the bar (grown/shrunk segment, or a color change) is redrawn each tick, never the whole bar cleared and refilled.
- Traces to: REQ-DISP-002, REQ-NFR-003.
