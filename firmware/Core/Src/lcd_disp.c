 ...
 *          previous and new fill width; text fields (except the large
 *          time display) are still redrawn in full each tick.
 *
 *          Time is rendered with the 48pt font from font_big.c/.h
 *          (generated by LCD-Image-Converter) instead of the driver's
 *          built-in ASCII bitmap font, since the built-in font's max
 *          size (24) wasn't large enough for a clearly-readable clock.
 *          The glyph-lookup and pixel-drawing helpers for that font live
 *          here (private to this module) rather than in a separate file,
 *          since lcd_disp is the only consumer -- design.md §3.3 only
 *          exposes lcd_disp_init()/lcd_disp_update() publicly anyway.
 */
#include "lcd_disp.h"
#include "lcd.h"
#include "font.h"
...
#define COLOR_BAR_FILL  0x0000  // yellow, or whatever distinct fill color you want
...
static void draw_glyph(uint16_t x, uint16_t y, const tImage *img, uint16_t color)
{
    ...
    // Clear this glyph's full bounding box to background first, so a
    // narrower new character doesn't leave remnants of a wider old one
    // (this was the cause of the overlapping/ghosting digits in the
    // photo -- previously only "on" pixels were painted, "off" pixels
    // from the previous glyph were never cleared).
